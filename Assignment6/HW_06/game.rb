...
# Author Abhi Falcao
# Note the Exception handling in start_game method

class Game
  #	Make sure that all the instance variables in this class may only be READ and not WRITTEN to. 
  attr_reader :guesses_allowed, :current_guess_count, :current_guess,:name, :secret_number, :set_of_numbers
  ...
  def start_game
    ...
    puts "Hi #{@name} you have #{guesses_allowed} to guess the secret number between #{@set_of_numbers.min} to #{@set_of_numbers.max}."
    ...
    while @current_guess_count < @guesses_allowed do
      ...
      if user_input_number(guessed_number, @set_of_numbers)
        next
      end  
